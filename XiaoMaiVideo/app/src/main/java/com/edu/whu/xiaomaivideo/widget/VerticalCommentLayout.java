...
import com.edu.whu.xiaomaivideo.others.CommentApplication;
import com.edu.whu.xiaomaivideo.util.SizeUtils;
...
public class VerticalCommentLayout extends LinearLayout implements
    ...
    private List<SecondLevelComment> mCommentBeans;
    ...
    public void addCommentsWithLimit(List<SecondLevelComment> commentBeans, int limit, boolean more) {
        ...
        for (int i = 0; i < showCount; i++) {
            ...
            SecondLevelComment commentBean = commentBeans.get(i);
            if (childView == null) {
    ...
    public void addComments(List<SecondLevelComment> commentBeans) {
        if (commentBeans == null) return;
        this.mCommentBeans = commentBeans;
        int oldCount = getChildCount();
        if (oldCount <= 0) {
     ...
     */
    public void updateTargetComment(int position, List<SecondLevelComment> commentBeans) {
        int oldCount = getChildCount();
        for (int i = 0; i < oldCount; i++) {
            if (i == position) {
                View childView = getChildAt(i);
                SecondLevelComment bean = commentBeans.get(i);
     ...
     * 創建Comment item view
     */
    private View makeCommentItemView(SecondLevelComment bean, int index) {
     ...
     * 添加需要的Comment View
     */
    private void addCommentItemView(View view, SecondLevelComment bean, int index) {
     ...
     */
    private void updateCommentData(View view, SecondLevelComment bean, int index) {
        bindViewData(view, bean);
    }

    private View makeContentView(SecondLevelComment content, int index) {
        View view = View.inflate(getContext(), R.layout.item_comment_single_child_new, null);
        bindViewData(view, content);
        return view;
    }

    private void bindViewData(View view, final SecondLevelComment content) {
    ...
    public interface CommentItemClickListener {
        ...
        void onItemClick(View view, SecondLevelComment bean, int position);

        void onLikeClick(View layout, SecondLevelComment bean, int position);
