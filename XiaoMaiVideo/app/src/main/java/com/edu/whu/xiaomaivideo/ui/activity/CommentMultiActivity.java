...
import com.edu.whu.xiaomaivideo.adapter.CommentDialogMutiAdapter;
import com.edu.whu.xiaomaivideo.model.CommentEntity;
import com.edu.whu.xiaomaivideo.model.CommentMoreBean;
...
public class CommentMultiActivity extends AppCompatActivity implements BaseQuickAdapter.RequestLoadMoreListener {
    ...
    private List<FirstLevelComment> datas = new ArrayList<>();
    ...
    private void initData() {
        ...
        for (int i = 0; i < size; i++) {
            ...
            firstLevelComment.setTotalCount(i + size);

            List<SecondLevelComment> comments = new ArrayList<>();
            for (int j = 0; j < 10; j++) {
                ...
                secondLevelComment.setTotalCount(firstLevelComment.getTotalCount());
                comments.add(secondLevelComment);
                firstLevelComment.setSecondLevelComments(comments);
            }
            datas.add(firstLevelComment);
        }
    }
    ...
    private void dataSort(int position) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            //一级评论
            FirstLevelComment firstLevelBean = datas.get(i);
            if (firstLevelBean == null) continue;
            firstLevelBean.setPosition(i);
            posCount += 2;
            List<SecondLevelComment> secondLevelBeans = firstLevelBean.getSecondLevelComments();
            ...
            for (int j = 0; j < beanSize; j++) {
                SecondLevelComment secondLevelBean = secondLevelBeans.get(j);
                ...
                switch ((int) view1.getTag()) {
                    case CommentEntity.TYPE_COMMENT_PARENT:
                        if (view1.getId() == R.id.rl_group) {
                            ...
                            FirstLevelComment bean = (FirstLevelComment) bottomSheetAdapter.getData().get(position);
                    ...
                    case CommentEntity.TYPE_COMMENT_CHILD:
                        ...
                        if (view1.getId() == R.id.rl_group) {
                            ...
                            SecondLevelComment bean = (SecondLevelComment) bottomSheetAdapter.getData().get(position);
                            bean.setLikeCount(bean.getLikeCount() + (bean.getIsLike() == 0 ? 1 : -1));
                            bean.setIsLike(bean.getIsLike() == 0 ? 1 : 0);

                            List<SecondLevelComment> secondLevelBeans = datas.get((int) bean.getPosition()).getSecondLevelComments();
...
//                            CommentMultiActivity.this.dataSort(0);
                    ...
                    case CommentEntity.TYPE_COMMENT_MORE:
                        ...
                        SecondLevelComment secondLevelComment = new SecondLevelComment();
                        secondLevelComment.setContent("more comment" + 1);
                        secondLevelComment.setCreateTime(System.currentTimeMillis());
                        secondLevelComment.setHeadImg("https://ss1.bdstatic.com/70cFuXSh_Q1YnxGkpoWK1HF6hhy/it/u=1918451189,3095768332&fm=26&gp=0.jpg");
                        secondLevelComment.setId(1 + "");
                        secondLevelComment.setIsLike(0);
                        secondLevelComment.setLikeCount(0);
                        secondLevelComment.setUserName("星梦缘" + 1);
                        secondLevelComment.setIsReply(0);
                        secondLevelComment.setReplyUserName("闭嘴家族" + 1);
                        secondLevelComment.setTotalCount(moreBean.getTotalCount() + 1);

                        datas.get((int) moreBean.getPosition()).getSecondLevelComments().add(secondLevelComment);
                        CommentMultiActivity.this.dataSort(0);
                        bottomSheetAdapter.notifyDataSetChanged();
    ...
    private void addComment(boolean isReply, MultiItemEntity item, final int position, String msg) {
        ...
        if (position >= 0) {
            ...
            if (item instanceof FirstLevelComment) {
                ...
                positionCount = (int) (firstLevelComment.getPositionCount() + 1);
                pos = (int) firstLevelComment.getPosition();
                replyUserName = firstLevelComment.getUserName();
            } else if (item instanceof SecondLevelComment) {
                SecondLevelComment secondLevelComment = (SecondLevelComment) item;
                positionCount = (int) (secondLevelComment.getPositionCount() + 1);
                pos = (int) secondLevelComment.getPosition();
                replyUserName = secondLevelComment.getUserName();
            }
            ...
            secondLevelComment.setHeadImg("https://ss1.bdstatic.com/70cFvXSh_Q1YnxGkpoWK1HF6hhy/it/u=3370302115,85956606&fm=26&gp=0.jpg");
            secondLevelComment.setCreateTime(System.currentTimeMillis());
            secondLevelComment.setIsLike(0);
            secondLevelComment.setUserName(userName);
            secondLevelComment.setId("");
            secondLevelComment.setPosition(positionCount);

            datas.get(pos).getSecondLevelComments().add(secondLevelComment);
            CommentMultiActivity.this.dataSort(0);
            bottomSheetAdapter.notifyDataSetChanged();
            ...
            //添加一级评论
            FirstLevelComment firstLevelBean = new FirstLevelComment();
            ...
            firstLevelBean.setSecondLevelComments(new ArrayList<SecondLevelComment>());
